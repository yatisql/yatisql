...
impl TabularFileReader {
    ...
    pub fn new(path: &str) -> anyhow::Result<Self> {
        Ok(TabularFileReader {
            ...
            eol_flag: false,
            delimiter: Self::derive_delimiter(path), // default, will be derived later
        })
    }
    ...
    fn load_buffer(&mut self, current_pointer: usize) -> Result<(), IoError> {
        if self.eof_pointer < BUFFER_SIZE {
            // ┬──┬ ノ(ò_óノ)
            return Err(IoError::EndOfFile { path: self.path.clone() });
        }
        ...
        if available_buffer.is_empty() {
            ...
            return Err(IoError::LineTooLong { path: self.path.clone() });
        }
    ...
    fn next_str(&mut self) -> Result<&str, IoError> {
        ...
        self.eol_flag = true;
        unsafe { Ok(std::str::from_utf8_unchecked(&self.buffer[self.buffer_pointer..self.eof_pointer])) }
    }
